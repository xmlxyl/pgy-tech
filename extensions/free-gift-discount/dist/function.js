...
var THRESHOLD = 99;
var DEFAULT_COMBO_DISCOUNT_RULES = [
  {
    message: "SUMMER15",
    percentage: 15,
    required: [
...
function buildFreeGiftCandidates(lines, giftContext) {
  return lines.filter(
    (line) => isFreeGift(line, giftContext) && isFreeGiftRegionEligible(line, giftContext)
  ).map((line) => ({
...
function buildComboDiscountCandidates(lines, rules) {
  ...
  rules.forEach((rule) => {
    ...
    uniqueLines([...selectedRequiredLines, ...targetLines]).forEach(
      (line) => consumeLine(remainingQuantities, line)
    );
    candidates.push({
...
function selectRequiredLines(lines, remainingQuantities, requiredMatchers) {
  ...
  for (const matcher of requiredMatchers) {
    const line = lines.find(
      (candidate) => getRemainingQuantity(remainingQuantities, candidate) > 0 && !selected.includes(candidate) && matchesLine(candidate, matcher)
    );
    if (!line) return null;
    selected.push(line);
  }
...
function selectTargetLines(lines, remainingQuantities, targetMatchers, fallbackLines) {
...
function parseComboDiscountRules(value) {
  ...
  return rawRules.map((rawRule, index) => normalizeComboDiscountRule(rawRule, index)).filter(Boolean);
}
...
function normalizeMatchers(values, explicitType) {
  return values.map((value) => normalizeMatcher(value, explicitType)).filter(Boolean);
}
...
function matchesProductId(merchandise, productId) {
  ...
  return merchandiseProductId === productId || merchandiseProductId?.endsWith(`/Product/${productId}`);
}
